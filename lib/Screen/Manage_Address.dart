...
class StateAddress extends State<ManageAddress> with TickerProviderStateMixin {
        ...
        if (!error) {
          ...
          for (int i = 0; i < addressList.length; i++) {
            if (addressList[i].isDefault == "1") {
              ...
              if (!ISFLAT_DEL) {
                // todo null check
                if(addressList.length >0 && addressList[i].freeAmt != null && addressList[i].deliveryCharge != null){
                                  ...
                                  child: RefreshIndicator(
                                    ...
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        print("checking now here ${addressList[index].street} and ${addressList[index].name}");
  ...
  void addAddressModel() {
    ...
    for (int i = 0; i < addressList.length; i++) {
      print("ok now here ${addressList[i].name} and ${addressList[i].street}");
      addModel.add(RadioModel(
          isSelected: i == selectedAddress ? true : false,
          name: "${addressList[i].name!}, ${addressList[i].mobile!}",
          add:
              "${addressList[i].building!}, ${addressList[i].area!}, ${addressList[i].city!}, ${addressList[i].state!}, ${addressList[i].country!}, ${addressList[i].pincode!}",
