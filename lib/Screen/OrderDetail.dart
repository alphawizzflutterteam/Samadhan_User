...
class StateOrder extends State<OrderDetail>
  ...
  //   String? userids = prefs.getString("id");
  //   print("checking ids here ${userids}");
  //   _isNetworkAvail = await isNetworkAvailable();
  //   if (_isNetworkAvail) {
  //     try {
  //       var parameter = {USER_ID: userids, SAVE_LATER: save};
  //       print("save for later params ${parameter}");
  //       Response response =
  //       await post(getCartApi, body: parameter, headers: headers)
  //           .timeout(Duration(seconds: timeOut));
  //       var getdata = json.decode(response.body);
  //       print("response of save for later here ${getdata}");
  //       bool error = getdata["error"];
  //       String? msg = getdata["message"];
  //       if (!error) {
  //         var data = getdata["data"];
  ...
  //   SharedPreferences prefs = await SharedPreferences.getInstance();
  //   String? uids = prefs.getString("id");
  //   _isNetworkAvail = await isNetworkAvailable();
  //   if (_isNetworkAvail) {
  //     try {
  ...
  //           context.read<CartProvider>().removeCartItem(vid!);
  //           setState(() {
  //             saveLater = false;
  //           });
  ...
  //           saveLaterList.removeWhere((item) => item.varientId == vid);
  //           setState(() {
  //             addCart = false;
  //           });
  //           oriPrice = oriPrice + price;
    ...
    if (_isNetworkAvail) {
      ...
      if (!error) {
        ...
        List<SectionModel> cartList = (data as List)
            ...
            .toList();
        context.read<CartProvider>().setCartlist(cartList);
        print("total price is here ${totalPrice}");
        print("promo code here" + getdata[PROMO_CODES].toString());
      } else {
        if (mounted)
          setState(() {
            _isNetworkAvail = false;
  ...
  reOrder() {
    ...
    return MaterialButton(
        ...
        onPressed: () async {
          ...
          //   for(var i=0;i<variationIdList.length;i++){
    ...
    //       mainAxisAlignment: MainAxisAlignment.spaceBetween,
    //       children: [
    //         Text("Reorder Now",style: TextStyle(fontWeight: FontWeight.w600),),
    //         Switch(
