...
class StateMyOrder extends State<MyOrder> with TickerProviderStateMixin {
      ...
      try {
        if (isLoadingmore) {
          ...
          if (CUR_USERID != null) {
            ...
            print("get order list here ${getdata}");
  ...
  productItem(int index, OrderItem orderItem, OrderModel order) {
    if (orderItem != null) {
      ...
      if ((orderItem.listStatus!.last) == DELIVERD)
        back = Colors.green;
      else if ((orderItem.listStatus!.last) == SHIPED)
        back = Colors.orange;
      else if ((orderItem.listStatus!.last) == CANCLED ||
          orderItem.listStatus!.last == RETURNED)
        back = Colors.red;
      else if ((orderItem.listStatus!.last) == PROCESSED)
        back = Colors.indigo;
      else if (orderItem.listStatus!.last == WAITING)
        back = Colors.black;
      else
        back = Colors.cyan;
      print("order delivery price ${orderItem.price}");
      return Card(
        elevation: 0.1,
